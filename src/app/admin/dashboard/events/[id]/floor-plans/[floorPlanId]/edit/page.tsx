...
export default function EditFloorPlanPage() {
  ...
  // Combine predefined and custom elements
  ...
  useEffect(() => {
    const fetchFloorPlan = async () => {
      try {
        const response = await fetch(`/api/admin/events/${params.id}/floor-plans/${params.floorPlanId}`)
        ...
        if (response.ok) {
          ...
          toast.error(errorData.error || 'Failed to load floor plan')
          router.push(`/admin/dashboard/events/${params.id}/floor-plans`)
        }
      } catch (error) {
        console.error('Error fetching floor plan:', error)
        toast.error('An error occurred while loading the floor plan')
        router.push(`/admin/dashboard/events/${params.id}/floor-plans`)
      } finally {
        setLoading(false)
      }
  ...
  }, [params.id, params.floorPlanId, router, isClient])
  ...
  const handleAddElement = (type: ElementType, options?: Partial<ElementOption>) => {
    ...
    const newElement: FloorPlanElement = {
      id,
      type,
      x: 400 - (option.defaultWidth / 2), // Center in canvas
      y: 300 - (option.defaultHeight / 2),
    ...
    if (type === 'section' && options?.shape) {
      newElement.shape = options.shape as 'rectangle' | 'circle'
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errorData = await response.json()
        toast.error(errorData.error || 'Failed to update floor plan')
      }
    } catch (error) {
      ...
      <Modal isOpen={showSectionCreator} onClose={() => setShowSectionCreator(false)}>
        <SectionCreator 
          ...
          onTablesAddedToSection={(sectionId, tables) => {
            // Add containerId to all tables and add them to the elements array
            const tablesWithContainer = tables.map(table => ({
              ...table,
              containerId: sectionId
            }))
            
            // Add the tables to the elements array
