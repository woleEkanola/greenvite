...
import { ChevronLeft, Filter, RefreshCw, Send } from 'lucide-react'
...
export default function EventSentInvitesPage({ params }: { params: { id: string } }) {
  ...
  const [filterOptions, setFilterOptions] = useState({
    status: 'all',
    rsvpStatus: 'not_responded', // Default to show those who haven't RSVPed
    type: 'all'
  })
  ...
  const applyFilters = () => {
    ...
    if (filterOptions.rsvpStatus === 'not_responded') {
      filtered = filtered.filter(invite => invite.rsvpStatus === 'not_responded')
    } else if (filterOptions.rsvpStatus === 'attending') {
      filtered = filtered.filter(invite => invite.rsvpStatus === 'attending')
    } else if (filterOptions.rsvpStatus === 'pending') {
      filtered = filtered.filter(invite => invite.rsvpStatus === 'pending')
    }
    
    // Filter by type
    if (filterOptions.type !== 'all') {
      filtered = filtered.filter(invite => invite.type === filterOptions.type || invite.type === 'both')
    }
