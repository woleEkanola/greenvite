...
// GET /api/superadmin/events/[id]/admins - Get all admins for an event
...
// PUT /api/superadmin/events/[id]/admins - Update admins for an event
export async function PUT(
  ...
  try {
    ...
    const currentAdminIds = currentAdmins.map((admin: { userId: string }) => admin.userId);

    // Determine which admins to add and which to remove
    const adminsToAdd = adminIds.filter((userId: string) => !currentAdminIds.includes(userId));
    const adminsToRemove = currentAdminIds.filter((userId: string) => !adminIds.includes(userId));
